...
### Poster fee double-debit on task post
- **Context**: On the live testnet, posting a task causes the poster's balance to decrement by 2× the task budget. Observed twice: first run with task `a2b588c8...` (prior step-2 verification) and again with task `b2f96181...` (step-2 second e2e). A 500,000,000 µAET escrow yields a 1,000,000,000 µAET poster balance decrement on both runs. The pattern is reproducible, agent-independent, and task-independent.
- **Wrong approach**: Assuming the v4.1 settlement split (73/23/2/2 of the escrowed budget) accounts for all economic flow on a task. Under that assumption the poster should only lose `budget * (1 - refund_share)`, where `refund_share` depends on the verdict. On accept the poster should lose 500M (100% of escrow distributed to worker/validators/gen-ledger/treasury); on fail they should lose 135M (27% after 73% refund); on dispute they should lose 250M (50/50 split). None of those match the observed 1,000,000,000 µAET decrement.
- **Right approach**: There is an additional 500,000,000 µAET poster-side debit happening outside the escrow + settlement path. Source was not identified during step-2 verification. Open a focused "poster fee accounting" audit before any further economic-layer workstreams (challenge path, data ingestion, mainnet planning). The audit must (1) trace every debit against a poster's balance from task-post through settlement, (2) identify the unaccounted 500M µAET path, (3) either justify it (posting fee, router-assignment fee, etc.) and document it in the v4.1 economic model, or fix it if it is a bug.
- **Why it matters**: Economic correctness is the most load-bearing layer of the protocol. A protocol where poster balances decrement mysteriously is one nobody trusts. The math must be exact to the µAET with every flow documented — this is a CLAUDE.md invariant ("integer arithmetic only for economic calculations — exact to the µAET"). The bug is reproducible across two independent flows and is not specific to the agent or task; it is structural. This audit opens immediately after step 2 merges and blocks any workstream that touches the economic model.

### Claim path router-assignment lag
- **Context**: On the live testnet, worker `aet task claim` requests return HTTP 409 "pending router assignment" for roughly 60 seconds after a task is posted, despite Fast Path propagation completing in single-digit seconds. Step-2 verification observed 4 retries at 15-second intervals (61.6 seconds total) before the claim succeeded on task `b2f96181...`.
- **Wrong approach**: Assuming Fast-Path post propagation makes a task immediately claimable. The poster's task event propagates in seconds, but the router-availability assignment that backs `claim` is a separate, lazy process with its own timing.
- **Right approach**: Worker auto-claim flows must either be patient (60+ seconds with retry backoff) or accept that first-attempt failure is normal. The fix shape: either eager router assignment synchronously on task-post (so claim is immediately valid), or correct the 409 retry-after hint to match the actual lag window. Not blocking step 2; flagged for a focused fix in a follow-up workstream.
- **Why it matters**: Machine-speed claim is part of the experience standard for the protocol (principle 2 — "machine speed is the standard"). A 60-second lag between post and claim is human-tolerance, not machine-tolerance. The gap between propagation speed and claim-readiness is exactly the kind of hidden timing assumption the protocol is designed to avoid. Surface it explicitly either in the error message or in the architecture; do not leave it as folklore that workers are expected to discover through trial and error.

---

## Static Analysis and Heuristic Design

### Projection lint: named-field wrapper evasion is a known gap bounded by code review
- **Context**: Step 3 of the reputation/consensus-integrity workstream shipped a projection-registry lint (`internal/projections/lint/`) that enforces plan §9.7 defense item 1 (CI static check for writer-without-caller) and plan §9.4 PR-3 (IntegrationTestRef must resolve). The §D3 heuristic catches stores whose struct has a direct `*badger.DB` field OR a truly **embedded** (anonymous) field that transitively contains `*badger.DB` — the wrapper-type evasion pattern `type Outer struct { *inner }` is caught. A first-cut implementation also recursed through **named** pointer fields (`type T struct { store *BadgerStore }`); this version produced 5 false positives on the real repo by flagging legitimate consumer types (`RoundCountConsumer`, `TaskVerificationConsensusConsumer`, `SlashingEvaluator`) that hold reader-references to stores rather than owning the persistence dependency.
- **Wrong approach**: Treating any named field holding a store pointer as suspect. This conflates "references a store" with "is a store." Consumers, evaluators, and settlers routinely hold named pointers to stores they read from or write through — flagging them as unregistered projections floods the lint with false positives. False positives at scale erode contributor trust and create pressure to suppress legitimate matches. The lint becomes noise that reviewers learn to ignore.
- **Right approach**: Tighten the embedding-detection rule to anonymous-field embedding only. A contributor who wants to evade registration via a named-field wrapper (`type Outer struct { wrapped *innerStore }`) sidesteps the static analyzer, but creating a wrapper type is a visible architectural choice that a reviewer sees on the same commit. The lint diagnostic explicitly states it is "the CI heuristic from plan §9.3, an approximation of the principle-level definition," and names code-review as the complement. SSA taint analysis or interface-resolution detection would close this gap structurally and is deferred to Step 3.5 per the D3 cost gate; until then, the named-field-wrapper path relies on code review as backstop.
