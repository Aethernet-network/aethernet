...
**Median score threshold**: removed. The original design imported a 6000 BP acceptance threshold from the legacy single-validator PassThreshold without calibrating it for multi-validator analyzer family score distributions. The analyzer families measure structurally different things on different scales (e.g., embedding_similarity runs naturally lower than statistical_structural for prose content), and a single cross-family median is a category error. Score values are preserved in the TaskVerificationConsensus payload as observability metadata for diagnostics, reputation, dispute analysis, and future analyzer-specific quality bars, but are not used to gate acceptance. See `docs/lessons.md` for the full lesson.

**Participation floor**: in addition to the diversity floor, acceptance requires that at least 3 distinct analyzer families have contributed any vote (pass, fail, or abstain) to the round. This is configurable per deployment via `DefaultParticipationFloor`, with an automatic adjustment for small networks: if the validator set has fewer than 3 total configured analyzer families, the floor reduces to `max(2, total_configured - 1)`. The participation floor makes 2-family cartel coordination structurally harder by requiring broader analyzer coverage on every round. Per Grok's adversarial review (April 2026).
