...
**Version**: v2 — post Gate 5B Plan v1 multi-AI review (Grok + ChatGPT). 12 changes applied (7 structural + 5 precision) plus 2 forward-note items.
**Status**: Draft. Pre-CC-plan-mode-review. Pre-founder-approval. Architect-reviewed (rounds 1 + multi-AI 2).
...
10. **Float-path removal (companion-PR scope decision)**: float-path removal — `computeValidatorPayoutsFloat`, `shadowMode` branches, float remainder absorption — lands as a **companion PR**, not in 5B proper. 5B's scope stays focused on pure derivation + applicator refactor. Architect-confirmed at Gate 5B Plan v1 review per Grok recommendation. The companion PR can land alongside 5B (same merge unit) or post-5B; architect decision at v3 review. F5 plan v3 §3.5's "float-path-excision-as-consequence-of-F5" is satisfied either way; 5B does not BLOCK on float-path removal but also doesn't itself perform the excision.
...
9. **Return `DerivationResult{Status: StatusDerived, Records: records, TerminalStatus: terminal, ResolvedCutoffAnchor: cutoff, ResolvedCutoffAnchorIsNil: isNil, Summary: summary}`** (or the verdict-appropriate terminal status).
...
**End of F5 Phase 5B Plan v2 — post multi-AI review. Ready for Claude Code plan-mode review.**
